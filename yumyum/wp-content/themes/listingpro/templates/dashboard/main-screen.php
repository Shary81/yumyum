<?php
	...
	if($pending_listings > 1){
		$notice = 'You have '.$pending_listings.' listings that are almost ready to publish.';
	}elseif($pending_listings == 1){
		$notice = 'You have '.$pending_listings.' listing that is almost ready to publish.';
	}
?>
								...
								echo '
									<li>
										...
										<span>
											...
											Someone reacted as <strong>'.$val["rating"].'</strong> '.esc_html__('on a comment by ','listingpro').' '.get_the_author_meta('user_login',$val['reviewer']).'.
											<time>'.$val["time"].'</time>
										</span>
									</li>
								';
						...
						if($rating <= 3){
							echo '
								<li>
									...
									<a class="reply" href="'.$currentURL.$perma.$dashQuery.'reviews#comment-'.get_the_ID().'">Reply</a>
								</li>
							';
						}
					}
