		...
		if ( $query->have_posts() ) { ?>
					...
					<?php
						while ( $query->have_posts() ) {
							...
							<?php
								...
								$results = $wpdb->get_results( "SELECT * FROM ".$dbprefix."listing_campaigns WHERE user_id ='$user_id' AND post_id='$postID' AND status = 'success'" );
							...
							<div class="table-responsive campaign">
								<div class="top-section">
									<h3><div class="listing_pro"><?php esc_html_e('Ad','listingpro'); ?></div> <span><?php the_title(); ?></span> <span class="active-status active"><?php esc_html_e('Active','listingpro'); ?></span></h3>
											...
											if(!empty($campaigns)){
												foreach($campaigns as $camp){
													if($camp=="lp_random_ads"){
														$typetitle = "Random Ads";
													}
													if($camp=="lp_detail_page_ads"){
														$typetitle = "Business Detail page";
													}
													if($camp=="lp_top_in_search_page_ads"){
														$typetitle = "Top in Search & Taxonomies";
													}
								...
								<table class="table table-striped">
									<thead>
										<tr>
											...
											<th><?php esc_html_e('Ammount','listingpro'); ?></th>
									...
									<tbody>
										<tr>
											...
											<td><?php echo $ads_durations; ?> <?php esc_html_e('days','listingpro'); ?></td>
			...
			?>
				<div class="text-center no-result-found col-md-12 col-sm-6 col-xs-12 margin-bottom-30">
					...
					<p> <?php esc_html_e('Sorry ! Your have no active campaigns yet!','listingpro'); ?></p>
				</div>
			<?php
		}
