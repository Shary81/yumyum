	...
	if (!function_exists('ajax_login')) {
		function ajax_login(){
			...
			if ( is_wp_error($user_signon) ){
				...
				echo json_encode(array('loggedin'=>true, 'message'=>'<span class="alert alert-success"><i class="fa fa-check" aria-hidden="true"></i> '.esc_html__('Login successful, redirecting...','listingpro').'</span>', 'redirect'=> home_url('/members/yumyum/profile/')));
			}
	...
	if (!function_exists('ajax_register')) {
		function ajax_register(){
			...
			if ( !$user_id && email_exists($user_email) == false && !empty($user_email) && !empty($user_name) && is_email($user_email) && !empty($first_name)  && $gender!='gender' && $usertype!='usertype' && !empty($country)) {
				...
				$user_id = wp_create_user($first_name, $user_name, $random_password, $user_email, $gender, $country,$usertype );
			...
			$note = '<span class="alert alert-success"><i class="fa fa-check" aria-hidden="true"></i> '.esc_html__('Go to your inbox or spam/junk and get your password','listingpro').'</span>';		
				...
				$error = true;
				$note = '<span class="alert alert-danger"><i class="fa fa-times" aria-hidden="true"></i>'.esc_html__(' Country field is empty.','listingpro').'</span>';
			}
