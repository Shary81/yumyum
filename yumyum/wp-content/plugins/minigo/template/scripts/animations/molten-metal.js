 ...
 * URL    : http://codepen.io/tmrDevelops/pen/JGvyra
 */


var c = document.getElementById('js-animation');
var thecanvas = c.getContext('2d');
document.body.clientWidth;
var wh = 128;
var w2h = wh * wh;
c.width = c.height =  wh;
var img = thecanvas.createImageData(wh, wh);
var id = img.data;
var t = 0;
var inc = 1 / wh;
var arr = [];
...
function draw(){
  window.requestAnimationFrame(draw);
   ...
   thecanvas.putImageData(img, 0, 0);
}
function hue(thecanvas) {
   return 255 * Math.min(Math.max(thecanvas, 0), 1);
}
function ease(x) {
   return (x > 0.2) ? 0 : i(1, 0, x * 6);
}
var db = document.body;
function i(thecanvas, db, t) {
   t = t * t * t * (6 * t * t - 15 * t + 10);
   return thecanvas + (db - thecanvas) * t;
}
function n(x, y) {
