...
if (!function_exists('settings_array')) {
    function settings_array($settings){ ?>
        <tr id="lp_field_<?php echo wp_kses_post($settings['id']); ?>">
            ...
            <td>
                <input type="text" name="<?php echo wp_kses_post($settings['id']); ?>" id="<?php echo wp_kses_post($settings['id']); ?>" value="<?php print_r($settings['value']); ?>" />
            </td>
        </tr><?php
    }
}
	...
	<th>FAQ helps to understand.</th>
		<td>
			<div id="tabs">
					  ...
					  <?php
						if( !empty($faq) && count($faq)>0 ){
						  foreach($faq as $key=>$sfaq){
							  echo '<li><a href="#tabs-'.$key.'">Question '.$key.'</a></li>';
						  }
						}else{
					  ?>
					<li><a href="#tabs-1">Q 1</a></li>
					<?php } ?>
					
				  </ul>
				  <a id="tabsbtn" class="lp-secondary-btn btn-first-hover">+ Add New</a>
				</div>
				<div class="clear"></div>
				 <?php
					if( !empty($faq) && count($faq)>0 ){
						foreach($faq as $key=>$sfaq){
						  echo '<div id="tabs-'.$key.'">
									<div class="form-group">
											<label for="inputLinkedIn">FAQ Number '.$key.'</label>
											<input type="text" class="form-control" name="faqs[faq]['.$key.']" id="inputLinkedIn" placeholder="FAQ '.$key.'" value="'.$sfaq.'">
									</div>
									<div class="form-group">
											<label for="inputDescription">Answer '.$key.'</label>
				...
				<div id="tabs-1">
					<div class="form-group">
							<label for="inputLinkedIn">FAQ Number 1</label>
							<input type="text" class="form-control" name="faqs[faq][1]" id="inputLinkedIn" placeholder="FAQ 1">
					</div>
					<div class="form-group">
							<label for="inputDescription">Answer 1</label>
    ...
    function settings_checkbox($settings){
        ...
        <tr id="field_<?php echo wp_kses_post($settings['id']); ?>" >
            ...
            <td >
                ...
                <input type="checkbox" class="yesno<?php echo wp_kses_post($klass); ?>" id="<?php echo wp_kses_post($settings['id']); ?>" data-show="<?php echo wp_kses_post($datashow);?>" data-hide="<?php echo wp_kses_post($datahide);?>" name="<?php echo wp_kses_post($settings['id']); ?>" value="Yes" <?php echo checked($default, 'Yes', false);?> />     
